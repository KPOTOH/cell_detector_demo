...
cell_square_statistics = [
    (85, 2938.2, 1592.9),
    (60, 4057.1, 2163.9),
    (20, 1655.7, 441.4),
]
trajectories_plots_path = [
    "data/Well_A1_05_plot.png",
    "data/Well_A1_08_plot.png",
    "data/Well_A2_25_plot.png",
]
...
def process_video():
    st.write("""## You can choose a video and see the predictions""")
    option = st.radio(
        '',
        [f'video {i + 1}' for i in range(3)],
    )
    ...
    try:
        ...
        #                         video_path.replace('Well', 'predicted_Well'))
        # tracks = get_track(video_prediction)

        predicted_video_path = video_path.replace('Well', 'predicted_Well')
    ...
    ncells, cell_square_mean, cell_square_std = cell_square_statistics[video_idx]
    st.markdown(
        f"""
        Detected **{ncells}** cells;
    ...
    # st.pyplot(fig)
    # fig.savefig(trajectories_plots_path[video_idx]) 

    plot_image = Image.open(trajectories_plots_path[video_idx])
    st.image(plot_image)
